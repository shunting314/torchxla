...
XLATensorPtr XLATensor::clamp(const XLATensorPtr& input,
  ...
  if (min) {
    res = torch::lazy::MakeNode<Maximum>(
        res, bridge::GetXLATensorPtr(*min)->GetIrValue(),
        std::vector<torch::lazy::Shape>());
  }
  if (max) {
    res = Min(res, bridge::GetXLATensorPtr(*max)->GetIrValue());
  }
...
XLATensorPtr XLATensor::erf(const XLATensorPtr& input) {
  return input->CreateFrom(Erf(input->GetIrValue()));
}

XLATensorPtr XLATensor::erfc(const XLATensorPtr& input) {
  return input->CreateFrom(Erfc(input->GetIrValue()));
}

XLATensorPtr XLATensor::erfinv(const XLATensorPtr& input) {
...
XLATensor::native_batch_norm(const XLATensorPtr& input,
  ...
  if (training) {
    ...
    at::Tensor at_input = bridge::AtenFromXLATensorPtr(input);
    mean = bridge::GetXLATensorPtr(at::empty({0}, at_input->options()));
    variance_inverse =
        bridge::GetXLATensorPtr(at::empty({0}, at_input->options()));
  }
