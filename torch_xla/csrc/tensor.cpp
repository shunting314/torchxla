...
=======
        ...
        // which will lead to incorrect results. We cannot normally turn A's
        // state
        // into DEVICE_DATA, as if any of the sources is a view, this will not
        // lead to correct results (as A's value taken at different times need
        // to
...
=======
      ...
      XLATensor::CompilationResult XLATensor::Compile(
        ...
        if (enable_aliasing && coll.config.sync_xla_data) {
          ...
          // normally turn A's state into DEVICE_DATA, as if any of the sources
          // is a view, this will not lead to correct results (as A's value
          // taken at different times need to reflect view source changes):
          //   1. Tensor A = some_graph_with_view_source(V)
          //   2. print(A)
          //   3. V += 1
          ...
          // But, when we issue a step barrier (force_xla_data == true) we have
          // to turn everything into DEVICE_DATA, so we can activate aliasing.
          BuildInputOutputAliases(tensors, coll.indices, &lowering_ctx);
        }
>>>>>>> e211ec20 (Update sharding spec to support full replication & mesh sharding)
...
TF_VLOG(3) << "Compiling IR graph hash " << torch::lazy::HashToString(coll.hash)
           << " on device " << coll.device << " ...";
std::vector<std::shared_ptr<xla::ComputationClient::Computation>> computations =
    xla::ComputationClient::Get()->Compile(std::move(instances));
TF_VLOG(3) << "Compiling IR graph hash " << torch::lazy::HashToString(coll.hash)
...
TF_VLOG(5)
    ...
    << torch::lazy::HashToString(torch::lazy::Hash(
           computations.front()->computation().proto().SerializeAsString()));
XLA_CHECK_EQ(program_shape.parameters_size(), po_data->parameters_data.size());
