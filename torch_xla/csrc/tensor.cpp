  ...
  std::vector<XLATensorPtr> GetLiveTensors(
    ...
    auto fn = [&](DeviceContext* devctx) {
      ...
      for (auto& uid_wptr : devctx->tensors_data) {
        ...
        if (data != nullptr) {
          tensors.push_back(c10::make_intrusive<XLATensor>(std::move(data)));
        }
      }
    };
...
XLATensorPtr XLATensor::Create(const at::Tensor& tensor,
  ...
  XLATensorPtr xtensor = c10::make_intrusive<XLATensor>(tensor, device);
...
XLATensorPtr XLATensor::Create(
  ...
  XLATensorPtr xtensor =
      c10::make_intrusive<XLATensor>(std::move(xla_data), logical_element_type);
  DeviceContextArena::Get()->RegisterTensor(xtensor->data_ptr());
  return xtensor;
}
...
XLATensorPtr XLATensor::Create(
  ...
  XLATensorPtr xtensor = c10::make_intrusive<XLATensor>(
      std::move(ir_value), device, logical_element_type);
  DeviceContextArena::Get()->RegisterTensor(xtensor->data_ptr());
...
XLATensorPtr XLATensor::Create(
    ...
    c10::optional<at::ScalarType> logical_element_type) {
  XLATensorPtr xtensor = c10::make_intrusive<XLATensor>(std::move(view), device,
