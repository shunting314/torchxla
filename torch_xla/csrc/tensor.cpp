...
>>>>>>> e211ec20 (Update sharding spec to support full replication & mesh sharding)
    ...
    void XLATensor::SetInPlaceIrValue(torch::lazy::Value ir_value) {
      ...
      if (xla_shape.get().element_type() !=
        ...
        ir_value = torch::lazy::MakeNode<Cast>(ir_value,
                                               xla_shape.get().element_type());
      }
    ...
    void XLATensor::TryLimitGraphSize() {
      ...
      if (data()->ir_value &&
          ++g_tls_data.trim_counter % kCheckFrequency == 0) {
        size_t graph_size =
    ...
    torch::lazy::Value XLATensor::GetIrValue() const {
      ...
      torch::lazy::BackendDataPtr xla_data = CurrentXlaData();
      if (xla_data != nullptr) {
        // In case of tensor node, we do not clear the XLA data when we set the
        // IR node. This because we want further calls to GetIrValue() to fetch
        // the same IR node, and not create new ones (even though the lowering
        // context will still collapse them all into a single XLA parameter op).
    ...
    torch::lazy::Value XLATensor::GetDeviceDataIrValue(
      ...
      UnwrapXlaData(data)->SetInfo(std::make_shared<DeviceDataInfo>(
          /*tensor_id=*/-1, /*read_only=*/true));
      return torch::lazy::MakeNode<DeviceData>(std::move(data));
    }

    torch::lazy::Value XLATensor::GetIrValueForConstant(
    ...
    torch::lazy::Value XLATensor::GetIrValueForScalar(
        const at::Scalar& value, const xla::Shape& shape,
        const torch::lazy::BackendDevice& device) {
      return GetIrValueForScalar(value, shape.element_type(),
                                 shape.dimensions(), device);
    }
    ...
    std::shared_ptr<View> XLATensor::CreateView(ViewInfo view_info) const {
      ...
      // This node is not a view, and creating a view forks the current node
      // into becoming one itself. This means creating an alias with the current
      // IR XlaNode, and using the same alias for the created IR XlaNode.
      torch::lazy::Value ir_value = GetIrValue();
      std::shared_ptr<Alias> alias = std::make_shared<Alias>(ir_value);
      ...
      if (!tensor_data) {
        ...
        if (detached) {
          if (data()->ir_value || data()->xla_data != nullptr ||
            ...
            // Otherwise we need to make a copy to prevent the caller changing
            // our version.
...
>>>>>>> e211ec20 (Update sharding spec to support full replication & mesh sharding)
      ...
      XLATensor::SyncTensorCollection XLATensor::CollectSyncTensors(
        ...
        // The force_xla_data controls aliasing compilation, so effectively the
        // same graph with on/off force_xla_data should not match, hash wise.
        ...
        if (!at_tensors.empty()) {
          ...
          for (size_t i = 0; i < handles.size(); ++i) {
            ...
            // tensor is not present. Also, we uploaded the at::Tensor data to
            // the device, but such data is still valid so we leave it live on
            // the XLA tensor (so that a following ToTensor() does not need to
            // fetch it from device).
            tensors[at_tensor_index[i]].data()->xla_data =
                std::move(handles[i]);
          }
        }
        TF_VLOG(4) << "Tensors graph hash "
      ...
      XLATensor::PostOrderData XLATensor::RunPostOrder(
        ...
        PostOrderData po_data;
        po_data.post_order =
            Util::ComputePostOrder(roots, &po_data.emission_map);
        ...
        for (auto node : po_data.post_order) {
          ...
          if (device_data != nullptr) {
            ...
            if (it != data_handles.end()) {
              ...
              po_data.parameter_sequence.push_back(
      ...
      std::vector<torch::lazy::BackendDataPtr> XLATensor::FetchTensorData(
        ...
        for (auto index : indices) {
          ...
          // If the config.force_xla_data flag is true, the purpose of this
          // tensor sync operation is to truncate the IR graph and materialize
          // device data in place of IR graph, on selected tensors. But since
          // operation will complete asynchronously, if a tensor does not
          // already have device data, we need to install a placeholder. Since
          // at this point we hold a lock on the device where the tensors reside
          // (locks held within the coll structure, and moved into the async
          // variable), any other operation trying to access the tensor's device
          // data will have to wait until the asynchronous operation completes.
          torch::lazy::BackendDataPtr xla_data = tensor.CurrentXlaData();
          if (xla_data == nullptr && config.force_xla_data) {
            const torch::lazy::BackendDevice& tensor_device =
                tensor.GetDevice();
            xla::Shape shape = MakeShapeWithDeviceLayout(
                tensor.shape(),
                static_cast<XlaDeviceType>(tensor_device.type()));
            xla_data = WrapXlaData(
                xla::ComputationClient::Get()->CreateDataPlaceholder(
      ...
      std::shared_ptr<XLATensor::Async> XLATensor::ScheduleSyncTensorsGraph(
        ...
        auto tensors_data =
            FetchTensorData(tensors, coll->config, coll->indices);
      ...
      void XLATensor::WaitDeviceOps(absl::Span<const std::string> devices) {
        ...
        // object, which is going to be freed immediately, turning this
        // operation into a lock barrier.
      ...
      XLATensor::OpByOpAsync XLATensor::SyncTensorsGraphOpByOp(
        ...
        TensorCollectionBarrier(&coll);
        auto async =
            std::make_shared<Async>(std::move(coll), std::move(tensors_data),
    ...
    std::shared_ptr<XLATensor::Async> XLATensor::ScheduleSyncTensorsGraph(
      ...
      void
      XLATensor::BuildInputOutputAliases(const std::vector<XLATensor>& tensors,
                                         absl::Span<const size_t> indices,
...
>>>>>>> e211ec20 (Update sharding spec to support full replication & mesh sharding)
    ...
    TF_VLOG(6) << "Aliased paramter " << i << " with output " << output_index
